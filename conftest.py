...
def web_browser(request, selenium):
    ...
    # Выполните демонтаж(этот код будет выполняться после каждого теста):
    ...
    if request.node.rep_call.failed:
        ...
        try:
            ...
            pass # just ignore any errors here
                 # просто игнорируйте любые ошибки здесь

        #Эта функция получает строку документа из тестового примера и
        # форматирует ее так, чтобы в отчетах отображалась эта строка
        # документа вместо имени тестового примера.
def get_test_case_docstring(item):
    ...
    if item._obj.__doc__:
        ...
        # Сгенерируйте список параметров для параметризованных тестовых примеров:
...
# Эта функция изменяет имена тестовых примеров "на лету" во время выполнения
# тестовых примеров.
...
# Эта функция изменяет имена тестовых примеров "на лету", когда мы используем
# параметр --collect-only для pytest (чтобы получить полный список всех
# существующих тестовых примеров).
def pytest_collection_finish(session):
    ...
    if session.config.option.collectonly is True:
        for item in session.items:
            ...
            # automatically import test cases to test management system.Если
            # тестовый пример имеет строку doc, нам нужно изменить его имя на
            # строку doc, чтобы показывать удобочитаемые отчеты и автоматически
            # импортировать тестовые примеры в систему управления тестированием.
